...
const ServicesSection = () => {
  ...
  return (
    <section className="bg-gray-300 py-16">
      <img src={section2} alt="services" className="mx-8 mb-8 rounded-sm max-w-[calc(100%-4rem)]" />
      ...
      <div className="max-w-8xl mx-auto px-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {cards.map((card, index) => (
          ...
          >
            ...
            <h3 className="text-xl font-bold text-black group-hover:text-white text-center">{card.title}</h3>
            <p className="font-medium text-sm text-center text-gray-700 group-hover:text-white">{card.desc}</p>
          </div>
        ))}
      </div>
    </section>
